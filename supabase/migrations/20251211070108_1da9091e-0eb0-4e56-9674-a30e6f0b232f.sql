-- Update withdraw_early_from_cycle to apply 18% tax on ALL cycles including Special
...
BEGIN
  ...
  -- Apply 18% tax on ALL cycles including Special cycle
  v_tax := v_current_value * (v_tax_rate / 100);
  v_final_amount := v_current_value - v_tax;
  
  -- Penalty mode only activates for cycles 1, 2, 3 (not Special)
