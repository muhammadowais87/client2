...
-- Cycles lock after completion, next cycle unlocks, chances stay available until Special
...
BEGIN
  ...
  LOOP
    ...
    IF v_cycle.cycle_type = 4 THEN
      -- Special cycle completed: Mark chance as completed and reset progress
      UPDATE public.user_trade_progress
      SET completed_cycles = '{}',  -- Reset to empty array for next round
          is_penalty_mode = false,
          last_50_percent_check = now(),
          active_chance = NULL,
          -- NOW mark the chance as completed since Special cycle is done
      ...
      WHERE user_id = v_cycle.user_id;
    ELSE
      -- Regular cycle completed: 
      -- Add to completed_cycles (to unlock next cycle)
      -- Keep chance as 'available' (NOT completed) - user can continue using same chance
      UPDATE public.user_trade_progress
      SET completed_cycles = array_append(COALESCE(completed_cycles, '{}'), v_cycle.cycle_type),
          is_penalty_mode = false,
          last_50_percent_check = now(),
          active_chance = NULL,
          -- Keep chance available, don't mark as completed until Special cycle
