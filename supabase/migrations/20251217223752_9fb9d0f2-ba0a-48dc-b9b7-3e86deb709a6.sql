-- Fix deactivate_chance to preserve penalty mode status when moving to next chance
...
BEGIN
  ...
  -- Store whether user was in penalty mode before deactivation
  ...
  IF p_chance_number = 1 THEN
    -- When deactivating chance 1, preserve penalty mode if user was in it
    -- They need to complete a full cycle to exit penalty mode
    UPDATE user_trade_progress
    SET chance_1_status = 'disabled',
        chance_2_status = 'available',
        active_chance = NULL,
        completed_cycles = '{}',
        -- PRESERVE penalty mode - only reset if NOT in penalty mode
  ...
  ELSE
    -- When deactivating chance 2, all chances are used
    UPDATE user_trade_progress
    SET chance_2_status = 'disabled',
        active_chance = NULL,
        completed_cycles = '{}',
        -- Keep penalty mode status as-is for chance 2
        updated_at = now()
    WHERE user_id = v_user_id;
